...
--
-- NOTA: Não usar CONCURRENTLY aqui porque golang-migrate
-- roda migrations dentro de transações, e PostgreSQL não
-- permite CREATE INDEX CONCURRENTLY em transações.
-- Em produção, adicionar índices manualmente com CONCURRENTLY.
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_membros_familia_id_active
ON membros (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_categorias_familia_id_active
ON categorias (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_cartoes_credito_familia_id_active
ON cartoes_credito (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_despesas_cartao_familia_id_active
ON despesas_cartao (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_assinaturas_familia_id_active
ON assinaturas (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_contas_fixas_familia_id_active
ON contas_fixas (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_despesas_gerais_familia_id_active
ON despesas_gerais (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_rendas_fixas_familia_id_active
ON rendas_fixas (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_rendas_variaveis_familia_id_active
ON rendas_variaveis (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_rendas_extras_familia_id_active
ON rendas_extras (familia_id)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_rendimentos_investimento_familia_id_active
...
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_usuario_id_active
...
CREATE INDEX IF NOT EXISTS idx_despesas_cartao_fatura
...
CREATE INDEX IF NOT EXISTS idx_despesas_cartao_data_compra
ON despesas_cartao (familia_id, data_compra)
WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_despesas_gerais_data
...
CREATE INDEX IF NOT EXISTS idx_rendas_fixas_vigencia
...
CREATE INDEX IF NOT EXISTS idx_categorias_created_at
ON categorias (familia_id, created_at DESC)
WHERE deleted_at IS NULL;

-- Cartões (lista ordenada por cadastro)
CREATE INDEX IF NOT EXISTS idx_cartoes_credito_created_at
