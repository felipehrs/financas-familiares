...
type MockRendaFixaService struct {
	CriarFn        func(descricao, membroID string, valor float64, diaRecebimento int) (*domain.RendaFixa, error)
	...
	AtualizarFn    func(id, descricao, membroID string, valor float64, diaRecebimento int, ativa bool) (*domain.RendaFixa, error)
	InativarFn     func(id string) error
}

func (m *MockRendaFixaService) Criar(descricao, membroID string, valor float64, diaRecebimento int) (*domain.RendaFixa, error) {
	return m.CriarFn(descricao, membroID, valor, diaRecebimento)
}
...
func (m *MockRendaFixaService) Atualizar(id, descricao, membroID string, valor float64, diaRecebimento int, ativa bool) (*domain.RendaFixa, error) {
	return m.AtualizarFn(id, descricao, membroID, valor, diaRecebimento, ativa)
}
...
func TestCriarRendaFixaHandler_DescricaoVazia(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{
		"descricao": "", "membro_id": "membro-1", "valor": 5000.00, "dia_recebimento": 5,
	})
	req := httptest.NewRequest(http.MethodPost, "/api/v1/rendas-fixas", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
...
func TestAtualizarRendaFixaHandler_Sucesso(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{
		...
		"valor": 6000.00, "dia_recebimento": 10, "ativa": true,
	})
...
func TestAtualizarRendaFixaHandler_NaoEncontrada(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{
		"descricao": "Salário", "membro_id": "membro-1", "valor": 5000.00, "dia_recebimento": 5, "ativa": true,
	})
